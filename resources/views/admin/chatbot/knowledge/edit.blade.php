...
<div class="max-w-2xl">
    <div class="bg-white rounded-lg border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-1">Edit Artikel</h2>
        <p class="text-xs text-gray-400 font-mono mb-5">slug: {{ $knowledgeArticle->slug }}</p>

        <form method="POST" action="{{ route('admin.chatbot.knowledge.update', $knowledgeArticle) }}" class="space-y-5">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $knowledgeArticle->category) }}"
                       list="category-list"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $knowledgeArticle->title) }}"
            ...
            <div>
                ...
                <textarea name="content" rows="10"
                          class="w-full border @error('content') border-red-400 @else border-gray-300 @enderror rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 font-mono">{{ old('content', $knowledgeArticle->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="keywords"
                       value="{{ old('keywords', $knowledgeArticle->keywordsAsString()) }}"
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" id="is_active" value="1"
                       @checked(old('is_active', $knowledgeArticle->is_active ? '1' : '0') === '1')
