...
class BookingReviewFormatterService
{
    ...
    public function buildCustomerReview(BookingRequest $booking, ?string $seed = null): string
    {
        ...
        return implode("\n", [
            ...
            'Penumpang : '.(($booking->passenger_count ?? 0) > 0 ? $booking->passenger_count.' orang' : '-'),
            'Seat : '.(($booking->selected_seats ?? []) !== [] ? implode(', ', $booking->selected_seats ?? []) : '-'),
            'Jemput : '.($booking->pickup_location ?? '-'),
            'Alamat jemput : '.($booking->pickup_full_address ?? '-'),
            'Tujuan : '.($booking->destination ?? '-'),
            'Nama : '.($booking->passengerNamesList() !== [] ? implode(', ', $booking->passengerNamesList()) : '-'),
            'Kontak : '.($booking->contact_number ?? '-'),
            'Total ongkos          : '.$this->fareCalculator->formatRupiah($this->resolvedFare($booking)).' (Ongkos akan dikonfirmasi ulang Admin Utama, mengingat untuk menyesuaikan Lokasi Jemput dan Pengantaran)',
            '',
            $this->variation->pick([
    ...
    public function buildAdminReview(BookingRequest $booking, string $customerPhone): string
    {
        $lines = [
            ...
            'Alamat tujuan antar  : '.($booking->destination_full_address ?? '-'),
